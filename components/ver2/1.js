...
const Ver2_1 = () => {
  ...
  const localIpAddress = '192.168.45.4';
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        await fetch('/api/socketio'); // ensure the server is initialized
      } catch {}
      socket = io(`http://${localIpAddress}:3000`, { path: '/socket.io', transports: ['websocket'] });

      socket.on('connect', () => {
        setMessage(`connected: ${socket.id}`);
        socket.emit('ping', 'Hello from ver2_1 (socket.io)!');
      });

      socket.on('welcome', (data) => setMessage(String(data)));
      socket.on('pong', (data) => setMessage(String(data)));
      socket.on('disconnect', () => setMessage('disconnected'));
      socket.on('connect_error', (err) => setMessage(`error: ${err.message}`));
  ...
  return (
    <div className="container" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '100%', height: '100vh', overflow: 'hidden' }}>
      ...
      <style jsx>{`
        ...
        .trail-blob { z-index: 0; }
        .trail-1 { z-index: 4; opacity: 0.7; animation-delay: -0.3s; filter: blur(17px); box-shadow: 0 0 0 1px rgba(100,255,175,0.25), 0 0 5px 1px rgba(100,255,175,0.15); }
        .trail-2 { z-index: 3; opacity: 0.5; animation-delay: -0.6s; filter: blur(19px); box-shadow: 0 0 0 1px rgba(100,255,175,0.2), 0 0 5px 1px rgba(100,255,175,0.1); }
        .trail-3 { z-index: 2; opacity: 0.35; animation-delay: -0.9s; filter: blur(21px); box-shadow: 0 0 0 1px rgba(100,255,175,0.15), 0 0 5px 1px rgba(100,255,175,0.08); }
        .trail-4 { z-index: 1; opacity: 0.22; animation-delay: -1.2s; filter: blur(23px); box-shadow: 0 0 0 1px rgba(100,255,175,0.1), 0 0 5px 1px rgba(100,255,175,0.05); }
        .trail-5 { z-index: 0; opacity: 0.12; animation-delay: -1.5s; filter: blur(25px); box-shadow: 0 0 0 1px rgba(100,255,175,0.07), 0 0 5px 1px rgba(100,255,175,0.03); }
        ...
        @media (max-width: 768px) {
          .blob-wrapper, .trail-blob { width: 400px; height: 400px; }
        }
